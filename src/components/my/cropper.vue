    ...
    <div>
      ...
      <div class="popup-footer">
        <div class="popup-footer-box">
          ...
          <el-button type="primary" @click="clickConfirm">上传</el-button>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      touchScale(e) {
        ...
        let oldL = Math.sqrt(Math.pow(oldTouch1.x - oldTouch2.x, 2) + Math.pow(oldTouch1.y - oldTouch2.y, 2))
        let newL = Math.sqrt(Math.pow(newTouch1.x - newTouch2.x, 2) + Math.pow(newTouch1.y - newTouch2.y, 2))
        let cha = ~~(newL - oldL)
